...
- Generate private key on my local device and save on user profile in Linux server
- Configure SSH configuration files - sshd_config and sshd_config.d to allow user authentication when connecting via ssh
...
- go to your local machine (laptop) and type the command in your terminal > ssh-keygen
- ![alt text](<Screenshot 2026-09-23 151355.png>)
- switch user on linux server to training and create a .ssh folder using>>  mkdir .ssh
...
- switch user out of training on your linux server- use user with sudo right to edit the sshd files in ssh folder.
...
Note: Make sure to restart your sshd service everytime to make changes to the configuration file
